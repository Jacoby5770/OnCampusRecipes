...
;;
(defn welcome [pmsg]
  (str "Welcome " (apply str (get (get pmsg :args) 0))))
...
(defn office-hours [{:keys [args cmd]}]
  (cond
    (contains? instructor-hours (apply str args)) (formatted-hours (get instructor-hours (apply str args)))
    :else "there are no office hours on that day"))
...
(defn action-send-msgs [people msg]
  (for [key people] (action-send-msg key msg)))
...
(defn recipes-register [recipes name cost duration cuisine]
  (action-insert [:recipes name] {:cost (read-string cost) :duration (read-string duration) :cuisine cuisine}))
...
(defn recipes-unregister [recipes name]
  (action-remove [:recipes name]))
...
(defn ask-experts [experts {:keys [args user-id]}]
  (cond
    (= (count experts) 0) [[] "There are no experts on that topic."]
    (< (count args) 2) [[] "You must ask a valid question."]
    :else [(into [] (concat (action-inserts [:conversations] experts user-id) (action-send-msgs experts (clojure.string/join " " (rest args))))) (experts-question-msg experts (rest args))]))
...
(defn answer-question [conversation {:keys [args]}]
  (cond
    (= (count conversation) 0)  [[] "You haven't been asked a question."]
    (= (count args) 0)  [[] "You did not provide an answer."]
...
(defn add-expert [experts {:keys [args user-id]}]
  [[(experts-register experts (first args) user-id nil)] (str user-id " is now an expert on " (first args) ".")])

(defn add-recipe [recipes {:keys [args user-id]}]
  [[(recipes-register recipes (first args) (second args) (nth args 2) (nth args 3))] (str "recipe has been added: " (first args) ".")])

(defn find-recipe [recipes {:keys [args user-id]}]
(println "*****************" recipes "*****************")
  (cond
    (= (count recipes) 0) [[]"No recipes exist in database."]
    :else [[](filter (fn [recipe] 
            (and (<= (:cost recipe) (read-string (first args)))
                (<= (:duration recipe) (read-string (second args)))
                (= (:cuisine recipe) (read-string (nth args 2)))
            ))
            recipes)]
          ))
...
(defn recipe-query [state-mgr pmsg]
   (get! state-mgr [:recipes]))
...
(defn create-router [routes]
  (fn [mes]
    (cond
      (contains? routes (get mes :cmd)) (get routes (get mes :cmd))
      :else (get routes "default"))))
